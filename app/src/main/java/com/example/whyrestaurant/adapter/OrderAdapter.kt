...
class OrderAdapter(val context: Context,  private val ordersTest: List<Order>): RecyclerView.Adapter<OrderAdapter.OrderItemViewHolder>() {

    override fun onCreateViewHolder(
        parent: ViewGroup,
        viewType: Int
    ): OrderAdapter.OrderItemViewHolder {
        return OrderItemViewHolder(LayoutInflater.from(context).inflate(R.layout.list_orders, parent, false))
    }

    @RequiresApi(Build.VERSION_CODES.O)
    override fun onBindViewHolder(holder: OrderAdapter.OrderItemViewHolder, position: Int) {
...
//    fun setOrders(data: List<OrderModel>){
//        orders.clear()
//        orders.addAll(data)
